 ...
 *
 */
public class CustomerMinConsumer implements Runnable{

	private Logger logger = Logger.getLogger(CustomerMinConsumer.class);
	private Map<String, List<Record>> map = null;
	
	public CustomerMinConsumer(){
	...
	public void run() {
		while(true) {
			try {
				if (!CustomerMinSingleton.getInstance().getCustomerMinQueue().isEmpty()) { // 如果文件队列不为空
					String filePath = CustomerMinSingleton.getInstance().getCustomerMinQueue().poll();
					...
					FileProcesser.moveFileToStat(filePath, GlobalSettings.STAT_CUSTOMER_HOUR);//MIN需要备份到stat文件夹，并生成.ok
